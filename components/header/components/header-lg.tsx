...
export default function HeaderLg() {
  ...
  onMouseLeave={() => setHide(true)}
      className={`fixed top-0 pt-[33px]left-0 w-full h-[125.31px] flex justify-center items-center backdrop-blur-md z-50 transition-transform duration-300 ${
        hide ? "-translate-y-[90%] opacity-100%"  : "translate-y-0"
      }`}
    >
      <div className="flex container justify-between items-center gap-10">
        ...
        <div className="flex justify-center items-center w-[680px] h-full gap-[33px]  bg-linear-to-b from-[#FEFFBC] to-[#FFC671]
          ...
          <button onClick={() => scrollTo("footer")} className="font-roboto text-[20px] tracking-[0.13em] uppercase hover:text-yellow-500 transition">community</button>
          <button onClick={() => scrollTo("support")} className="font-roboto text-[20px] tracking-[0.13em] uppercase hover:text-yellow-500 transition">support</button>
        </div>
